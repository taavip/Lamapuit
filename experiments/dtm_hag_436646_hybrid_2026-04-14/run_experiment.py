...
5) CHM creation at 0.2 m using last returns only, class exclusions, and strict HAG [0, 1.3].
...
def _smooth_masked_chm(arr: np.ndarray, nodata: float, sigma: float, hag_max: float) -> np.ndarray:
    if sigma <= 0:
        out = arr.copy()
        valid = out != nodata
        out[valid] = np.clip(out[valid], 0.0, hag_max)
        return out
    ...
    out[valid_out] = np.clip(out[valid_out], 0.0, hag_max)
    return out
...
def _generate_year_chms(
    ...
    with laspy.open(str(year_input.laz_path)) as fh:
        for pts in fh.chunk_iterator(chunk_size):
            ...
            for method_name, chm_arr in method_chms.items():
                ...
                # Keep soft-floor support but never write negative CHM values.
                valid_hag = np.clip(hag[keep_hag], 0.0, hag_max).astype(np.float32)
    ...
    for method_name, chm_arr in method_chms.items():
        ...
        if gaussian_sigma > 0:
            gauss_arr = _smooth_masked_chm(chm_arr, nodata=nodata, sigma=gaussian_sigma, hag_max=hag_max)
...
def _write_markdown_report(path: Path, payload: dict) -> None:
    ...
    lines.append("8. CHM from point-wise HAG with last-return filtering and class exclusion.")
    lines.append("")
    ...
    lines.append("- HAG constraint: keep only `HAG_min <= HAG_i <= HAG_max`, then clip output to [0, HAG_max]")
