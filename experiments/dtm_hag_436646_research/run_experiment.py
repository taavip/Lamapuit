...
def _smooth_masked(arr: np.ndarray, nodata: float, sigma: float, max_clip: float | None = None) -> np.ndarray:
    if sigma <= 0:
        out = arr.copy()
        if max_clip is not None:
            valid = out != nodata
            out[valid] = np.clip(out[valid], 0.0, max_clip)
        return out
    ...
    if max_clip is not None:
        ...
        out[valid_out] = np.clip(out[valid_out], 0.0, max_clip)
...
def _generate_year_chms(
    ...
    with laspy.open(str(year_input.laz_path)) as fh:
        for pts in fh.chunk_iterator(chunk_size):
            ...
            for method_name, chm_arr in method_chms.items():
                ...
                keep = (hag >= -2.0) & (hag <= hag_max)
                if not np.any(keep):
                    continue

                valid_flat = flat[valid_dtm][keep]
                valid_hag = np.clip(hag[keep], 0.0, None).astype(np.float32)
    ...
    for method_name, chm_arr in method_chms.items():
        ...
        smoothed = _smooth_masked(chm_arr, nodata=nodata, sigma=gaussian_sigma, max_clip=hag_max)
        _write_gtiff(
            sm_path,
            smoothed,
            spec["transform"],
...
def main() -> int:
    ...
    # 4) Build interpolators. (Use all points for IDW/TIN for high fidelity, downsample only for TPS)
