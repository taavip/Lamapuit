...
\subsubsection{CHM variantide võrdlus klassifitseerimises ja segmenteerimises}
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.95\linewidth]{estonian/joonised/vigade_visuaalsed_naited.png}
...
Käsitsi märgendatud valimile teostatud täiendav vigade analüüs paljastas, et vastupidiselt kõrgele keskmisele tulemile oli mudel võimeline tehiskavalust vigade tegema. Joonisel~\ref{fig:error-case-studies} on esitatud kaks ilustratiivset juhust: (i) \textbf{Valepostiivne} \textbf{(FP, usaldus 0,9889)} -- mudel arvas 128×128 paanil kuuluvat täielikult lamapuitu, kuigi sellel ei olnud peale struktureeritud müra ja madala taimestikku mingit signaali, mis näitab, et mudel võib konfundeerida lihtsa ruumilise korrapära kaunusega; (ii) \textbf{Valenegatiivsus (FN, ennustus 0,0249)} -- sama rasteris olev tegelik lamapuidupään jäi mudeli tähelepanu all täiesti märkamata, vihjates tugevale vastaste võimaluste osakaalule metsas kasvava võrastiku all. Esimene juhtum süstematiseerib meie jälgitut FP-riske (kraavid, rattarööpad jne.), teine aga selgitab FN-i loomust: andmete ebapiisav hajusus (1--4 pts/m² LiDAR) piisavalt hästi kirjeldab peent tüvedome kontuurit, mis on oluline tingimus, et mudel saaks õppida kindlakstegemiseks.

\begin{figure}[h!]
\centering
\includegraphics[width=0.95\linewidth]{estonian/joonised/error_cases_comparison.png}
\caption{Klassifitseerimismudelite edenamatust juhtumid käsitsi märgendatud valimis: (vasakul) valepostiivne paan, kus mudel arvas kõrgete usaldusega (98,89\%) lamapuitu, kuid tegelikult oli paan tühi; (paremal) valenegatiivsus, kus tegelik lamapuu pandi mudelile peaaegu nähtamatuks (2,49\% ennustus). Mõlemad juhtumid paanis 465663\_2023\_madal\_chm\_max\_hag\_20cm.tif.}
\label{fig:error-case-studies}
\end{figure}

\paragraph{Aktiivõppiõppe kaksepoolne õppimisprotsess.}

Analüüsitud vigade juhtumid (eriti valepostiivne kõrge usaldusega ja valenegatiivsus madala usaldusega) viitavad tõenäoliselt sellele, et need paanid märgistati käsitsi valesti -- õigepanesult märgendiks neid vastupidiselt. Selline olukord demonstreerib aktiivõppiõppe \textit{kaksepoolset õppimisprotsessi}: mudel ei õpi mitte ainult paranduste kaudu õigete ja valede ennustuste eristamisest, vaid töö autor omakorda \textit{õpib kiiremini ja süstemaatilisemalt} märgendama edaspidi samasuguseid struktuure. Järjepidevalt paanil 465663 esinevate vastastikuste vigade (kõrge FP ja madal FN samal rasteril) tõttu omandab annotaator selle konkreetse maastikutüübi märgendamise mustrid ning vältib edaspidi samade kutsete kordamist. Selline pöördteave mudelist annotatoorile on oluline osa aktiivõppega juhitud andmestiku koostamisel ning aitab lõppkokkuvõttes saavutada ühtlast ja kvaliteetset märgistusstandardi.
