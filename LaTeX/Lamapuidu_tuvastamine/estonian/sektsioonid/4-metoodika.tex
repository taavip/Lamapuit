...
\subsection{Andmete märgistamine ja aktiivõppe töövoog}
...
Lamapuidu usaldusväärseks ja efektiivseks märgistamiseks arendati käesoleva töö raames spetsiaalne brauseripõhine sildistamistarkvara, mille arhitektuur tugineb FastAPI raamistikul töötavale taustaserverile ning Konva.js teegiga realiseeritud interaktiivsele esiosale (HTML5 lõuendil). Loodud rakendus ühendas ja esitas sünkroniseeritult mitmeid ruumilisi andmekihte, eesmärgiga tasakaalustada märgistaja tähelepanu erinevate andmeallikate vahel ning toetada visuaalset otsustamist.

Sildistamistööriista tuumfunktsionaalsus hõlmas mitmeid töövoogu ja aktiivõpet optimeerivaid komponente:
\begin{itemize}
    \item \textbf{Intellektuaalne märgistamisjärjekord:} Serveri andmebaas töötles geomeetrilisi kriteeriume (kõrgusjaotus, servade tugevused) ja varajaste algmudelite ennustusi, tekitades dünaamiliselt tähtsuse järjekorra. See suunas eksperdi fookuse esmalt tiilidele, kus CWD esinemise tõenäosus oli suurim või mudeli hinnang kõige ebakindlam.
    \item \textbf{Ajaline ja tootevaade:} Sama asukohta sai vaadelda analüüsitava pildi ajarea kaudu (erinevate aastate skaneeringud) või toodete vaates (toorandmed vs. Gaussi silumisega CHM), mis lihtsustas juhuslikust mürast püsivate tüvede eristamist. 
    \item \textbf{Ruumilise konteksti kihid:} Lisaks madala taimkatte kõrgusmudelile kuvati 3D-varjundeid (\textit{hillshade}), Maa-ameti WMS-teenustest pärinevat ortofotot ja 3$\times$3 maatriksina naaberpiltide konteksti. Vajadusel hõlbustasid valideerimist ka kaldaerofotod või Google Street View välised lingid.
    \item \textbf{Pintsliga segmenteerimine (ingl \textit{brush segmentation}):} Tavapäraste piirdekastide (\textit{bounding box}) asemel kasutas sildistaja pidevalt muudetava läbimõõduga pintslitööriista, märkides lõuendil konkreetsed pikslid kaheklassilisse vormingusse (lamapuit, taust või ignoreeritav ala). Tulemus salvestati serverisse koos metaandmetega otse PNG-formaadis.
    \item \textbf{Mudeli soojuskaardi ülekate:} Esialgse mudeli arvutatud klassi aktiveerimiskaardid (ingl \textit{class activation maps - CAM}) visualiseeriti sildistamisprotsessis abistavate soojuskaartidena (režiimides nagu \textit{IntGrad}, \textit{HiResCAM} ja \textit{RISE}). Need suunasid spetsialisti tähelepanu potentsiaalsete objektide asukohtadele ning tõstsid eelmärgistamise kiirust.
\end{itemize}
