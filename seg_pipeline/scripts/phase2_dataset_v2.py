#!/usr/bin/env python3
"""Phase II V2: Adaptive Tiling & Dataset Construction with Conflict Masking.

Builds a patch index (CSV) from CHM variants with 3-fold vertical-stripe spatial
cross-validation, computes per-band statistics excluding conflict zones, and
provides the CWDSegDataset PyTorch Dataset used by Phase III V2.

3-fold spatial CV strategy:
    The 5000-column raster is divided into 3 vertical stripes of ~1667 cols each
    (= ~333m east-west ground extent per stripe at 0.2m/px).
    Stripe 0 (cols 0-1666, westernmost) is the permanently held-out test set.
    Folds 0-1 rotate stripes 1-2 as validation; remaining stripe = train.
    A 64-px buffer between train and val stripes is excluded from loss (valid=0).

Conflict zone masking (new in V2):
    Reads Band 3 (ensemble_prob) from the Phase I mask TIF.
    Excludes pixels where ensemble_prob ≥ 0.15 AND outside GPKG polygons
    (these are "conflict zones" — high ensemble confidence for CWD but no label).
    Only trains on: (a) GPKG polygons, (b) pixels with ensemble_prob < 0.15.

Usage:
    python phase2_dataset_v2.py --chm-variant baseline                  # build for variant
    python phase2_dataset_v2.py --chm-variant composite --validate       # stats + exit
"""
...
N_STRIPES = 3              # 3 stripes total (stripe 0 = test, stripes 1-2 = CV folds)
TEST_STRIPE = 0            # permanently held-out (westernmost)
STRIPE_WIDTH = TILE_WIDTH // N_STRIPES  # ~1667 columns per stripe
...
# Conflict zone thresholds (from Phase I mask synthesis)
CONFLICT_ENSEMBLE_THRESHOLD = 0.15  # pixels with ensemble_prob ≥ this are conflicts if no GPKG
...
class PatchEntry:
    row_off: int
    col_off: int
    stripe_id: int     # 0-2 (0 = test)
    fold_id: int       # which CV fold this is validation for (-1 if test)
    n_valid: int       # pixels where valid_mask == 1 after conflict masking
    n_positive: int    # pixels where target == 1 AND valid == 1


# ---------------------------------------------------------------------------
# 3-fold vertical-stripe spatial CV splitter
# ---------------------------------------------------------------------------


class SpatialCVSplitterV2:
    """Assigns fold IDs based on vertical stripe position (3-fold version).

    Stripe 0 (cols 0-1666)   → test set (fold_id = -1)
    Stripe 1 (cols 1667-3333) → fold 0 as validation, fold 1 as training
    Stripe 2 (cols 3334-4999) → fold 1 as validation, fold 0 as training
    """
    ...
    def __init__(
        ...
        # Stripes 1..N-1 become folds 0..N-2 (for 3-fold: only stripes 1,2 with folds 0,1)
    ...
    def train_val_split(
        ...
        val_fold: 0..1 — the fold index that is used as validation.
...
# ---------------------------------------------------------------------------
# Patch index construction with conflict zone masking
# ---------------------------------------------------------------------------


def _read_chm_path(variant: str, root: Path) -> Path:
    """Map CHM variant name to input symlink path."""
    variant_map = {
...
def _get_in_channels(variant: str) -> int:
    """Return expected number of channels for each variant."""
...
def _get_binary_bands(variant: str) -> list[int]:
    """Return 0-indexed list of bands to pass as-is (not normalized)."""
    if variant in ("baseline", "raw", "gauss"):
        return []  # single-band, normalize it
    elif variant == "masked":
        return [1]  # band 1 (0-indexed) is the mask, don't normalize
    elif variant == "composite":
        return [3]  # band 4 (0-indexed as 3) is validity mask
    else:
        return []
...
def build_patch_index(
    ...
    splitter: SpatialCVSplitterV2 | None = None,
    conflict_threshold: float = CONFLICT_ENSEMBLE_THRESHOLD,
) -> list[PatchEntry]:
    """Scan the raster in a dense grid and collect usable patch entries.

    Applies conflict zone masking: excludes pixels where ensemble_prob ≥
    conflict_threshold AND outside GPKG polygons.
    """
    if splitter is None:
    ...
    with rasterio.open(mask_tif) as msrc:
        for y0 in ys:
            for x0 in xs:
                ...
                # Band 1 = target, Band 2 = valid_mask, Band 3 = ensemble_prob
                raw = msrc.read(
                ...
                # Conflict zone masking: exclude pixels with high ensemble confidence but no GPKG
                # Keep: (1) inside GPKG (target=1), (2) low ensemble prob (outside GPKG)
                # Mask out: high ensemble prob but no GPKG (conflict zones)
...
class CWDSegDataset(Dataset):
    """Patch dataset for CWD semantic segmentation (V2).

    Reads 256×256 patches from a CHM variant and the Phase I true-mask raster.
    Applies joint geometric+radiometric augmentation + nodata dropout.

    Returns:
        image:  Tensor (C, 256, 256) float32, normalized
        target: Tensor (1, 256, 256) float32, {0, 1}
        valid:  Tensor (1, 256, 256) float32, {0, 1}  (buffer + conflict pixels zeroed)
    """
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        # Read phase1 mask (Band1=target, Band2=valid_mask, Band3=ensemble_prob)
        ...
        # Apply conflict zone masking: zero out high-prob ensemble pixels without GPKG
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Phase II V2: Dataset construction with conflict masking")
    p.add_argument(
        "--chm-variant",
        type=str,
        required=True,
        choices=["baseline", "raw", "gauss", "masked", "composite"],
        help="CHM variant to use for training",
    )
    p.add_argument(
        "--mask-tif",
        type=Path,
...
def main() -> None:
    ...
    # Band statistics from valid pixels (after conflict masking)
    if stats_path.exists():
        ...
        with rasterio.open(args.mask_tif) as msrc:
            ...
            ensemble_prob = raw[2].astype(np.float32)
            # Conflict mask: high ensemble prob + no GPKG polygon
