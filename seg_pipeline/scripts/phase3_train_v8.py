#!/usr/bin/env python3
"""Phase III V8: True SWA + v7c config (new 824-feature labels + soft targets).

V8 improves V7 with three critical fixes:

  1. True Stochastic Weight Averaging (Izmailov et al. 2018): maintains
     AveragedModel accumulating weight running mean over SWA phase. V7 only
     did LR scheduling; V8 does true weight averaging for flatter minima.

  2. Fixed SWA timing: swa_start_epoch=50 (V7: 70), patience=20 (V7: 12),
     epochs=120 (V7: 100). V7 folds stopped before epoch 70; V8 ensures
     ≥20 SWA updates for reliable weight averaging.

  3. v7c as default config (ablation winner): Tversky(α=0.3, β=0.7) +
     CLDice(λ=0.5) + soft distance-transform targets (σ=2.0). Ablation
     showed v7c > v7b by +0.023 F1 at fold 0.

  4. New 824-feature labels (cdw_labels_MP.gpkg): rasterized to updated
     406455_2021_tava_truemask.tif. 3× more CWD features; richer schema.

Usage:
    python phase3_train_v8.py --device cuda                    # v7c, all folds
    python phase3_train_v8.py --fold 0 --epochs 5 --device cuda # smoke test
    python phase3_train_v8.py --no-swa --device cuda            # disable SWA
"""
...
def train_fold(
    ...
    for epoch in range(1, epochs + 1):
        ...
        print(
            f"[V8|{variant}|fold{fold_id}] epoch={epoch:03d} "
    ...
    # Save SWA-averaged model if SWA ran (epoch >= swa_start_epoch)
    ...
    if swa_ran:
        ...
        print(f"  Saved SWA-averaged model to {fold_dir / 'swa_model.pt'}")
...
def run_all_folds(variant, chm_tif, mask_tif, band_stats, patch_index,
    ...
    for fold_id in range(n_folds):
        ...
        print(f"\n{'='*70}\n[V8|{variant}] Fold {fold_id}/{n_folds-1} — "
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Phase III V8: True SWA + v7c config")
    ...
    p.add_argument("--dataset-dir", type=Path,
                   default=ROOT / "seg_pipeline" / "output" / "phase2_dataset_v8")
    p.add_argument("--output-dir", type=Path,
                   default=ROOT / "seg_pipeline" / "output" / "phase3_runs_v8")
    ...
    p.add_argument("--soft-targets", action="store_true", default=True,
                   help="Use Gaussian distance-transform soft targets (default=True for V8)")
...
def main() -> None:
    ...
    print(f"Epochs: {args.epochs}, Patience: {args.patience}, SWA: start={swa_start_epoch} "
          f"({'disabled' if args.no_swa else 'enabled'})")
