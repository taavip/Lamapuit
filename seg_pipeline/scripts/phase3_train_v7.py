#!/usr/bin/env python3
"""Phase III V7: Blob-fix training — flipped Tversky + CLDice + soft targets.

V7 addresses the precision/recall imbalance found in V6 (precision=0.11,
recall=0.46) where the model predicts oversized blobs instead of thin CWD
structures.  Three targeted fixes:

  1. Flipped Tversky (α=0.3, β=0.7): β > α forces the model to be
     conservative — it must be confident before predicting CWD.  FN
     penalised 2.3× more than FP.  Reduces blob expansion.

  2. SoftCLDiceLoss (λ=0.5): Centerline Dice (Shit et al. 2021) computes
     Dice on the soft skeleton of pred/GT.  Penalises connectivity breaks
     and blob fragmentation that area-based losses ignore.

  3. Soft distance-transform targets (optional, --soft-targets): Replace hard
     binary masks with Gaussian heatmaps centred on the object medial axis.
     1-pixel offset = small gradient penalty instead of total failure.

Everything else retained from V6: U-Net++ EfficientNet-B2, 100 epochs,
patience=12, SWA from epoch 70, Mixup/CutMix/GridMask.

Ablation configs (selectable via CLI):
    v7a: TverskyFocal(0.3, 0.7) only
    v7b: TverskyFocal(0.3, 0.7) + CLDice(λ=0.5)     ← recommended default
    v7c: v7b + soft distance-transform targets

Usage:
    python phase3_train_v7.py --device cuda                    # v7b, all folds
    python phase3_train_v7.py --fold 0 --epochs 5 --device cuda # smoke test
    python phase3_train_v7.py --config v7a --device cuda        # ablation: Tversky only
    python phase3_train_v7.py --config v7c --device cuda        # ablation: full V7
"""
...
class V7CombinedLoss(nn.Module):
    """TverskyFocal(α=0.3, β=0.7) + λ·SoftCLDice.

    Precision-focused Tversky stops blob expansion; CLDice preserves thin-structure
    connectivity.  Default λ=0.5 weights the two terms equally on first run;
    ablate with λ=0.0 (V7a) or λ=0.5 (V7b/c).
    """

    def __init__(self, tversky_alpha: float = 0.3, tversky_beta: float = 0.7,
...
def train_fold(
    ...
    swa_scheduler = SWALR(optimizer, swa_lr=lr * 0.1, anneal_epochs=10,
                          anneal_strategy="cos") if swa_start_epoch < epochs else None
    ...
    for epoch in range(1, epochs + 1):
        ...
        print(
            f"[V7|{variant}|fold{fold_id}] epoch={epoch:03d} "
        ...
        if patience > 0 and no_improve >= patience and epoch >= 50:
    ...
    if epoch >= swa_start_epoch and swa_scheduler:
        print(f"  Updating batch norm for SWA...")
        def _swa_iter(loader):
            for b in loader:
                yield b["image"]
        update_bn(_swa_iter(train_loader), model, device=device)
...
def run_all_folds(variant, chm_tif, mask_tif, band_stats, patch_index,
                  output_dir, device, epochs=100, batch_size=None, patience=12,
                  num_workers=0, label_smooth=0.0, decoder_channels=None,
                  tversky_alpha=0.3, tversky_beta=0.7, cldice_weight=0.5,
                  soft_targets=False, soft_sigma=2.0,
                  swa_start_epoch=70, swa_update_freq=5) -> list[dict]:
    ...
    for fold_id in range(n_folds):
        ...
        print(f"\n{'='*70}\n[V7|{variant}] Fold {fold_id}/{n_folds-1} — "
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Phase III V7: Blob-fix training")
    p.add_argument("--variant", default=DEFAULT_VARIANT,
                   choices=["baseline", "raw", "gauss", "masked", "composite"])
    p.add_argument("--fold", type=int, default=-1)
    p.add_argument("--config", default="v7b", choices=["v7a", "v7b", "v7c"],
                   help="Ablation config: v7a=flip-Tversky only, v7b=+CLDice, v7c=+soft targets")
    ...
    p.add_argument("--output-dir", type=Path,
                   default=ROOT / "seg_pipeline" / "output" / "phase3_runs_v7")
    ...
    p.add_argument("--soft-targets", action="store_true",
                   help="Use Gaussian distance-transform soft targets instead of hard binary")
    p.add_argument("--soft-sigma", type=float, default=2.0)
    p.add_argument("--swa-start-epoch", type=int, default=70)
...
def main() -> None:
    ...
    # Apply ablation config shortcuts
    if args.config == "v7a":
    ...
    print(f"Epochs: {args.epochs}, Patience: {args.patience}, SWA: start={args.swa_start_epoch}")
    ...
    common_kw = dict(
        ...
        soft_sigma=args.soft_sigma, swa_start_epoch=args.swa_start_epoch,
